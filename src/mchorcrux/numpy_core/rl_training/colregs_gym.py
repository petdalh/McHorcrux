...
#-------------------------
    def evaluate_colregs_robustness(self):
        ...
        ego_state_raw = np.array([n, e, psi, ego_vn, ego_ve])
        tgt_state_raw = np.array([tgt_n, tgt_e, tgt_psi])
        trace = self.spec.build_signals(ego_state_raw, tgt_state_raw, self.ellipsoids_Ab_dict)
        rho, _, _ = self.spec.evaluate(trace)
        for r in rho:
            #print(f"Robustness interval: {type(r[1])}")
            if r[1].u > 0:
                print(f"  Robustness interval upper bound: {r[1].u:.4f}")
                #initiate crossing
            
        #print(f"eval robustness: {rho}")
    ...
    def set_encounter_task(
        ...
        encounter_type : str
            'crossing' – Target vessel approaches from starboard (right) on a
            perpendicular course.  Own ship is the give-way vessel and must
            yield by turning to starboard (COLREGs Rule 15).

        For 'crossing' the target is placed at a 45° bearing to starboard at
        'separation' metres, heading perpendicular across own ship's bow
        (own_heading − 90°).  This creates a collision course from the
        starboard side.
        """
        SUPPORTED_ENCOUNTERS = {"crossing"}
        if encounter_type not in SUPPORTED_ENCOUNTERS:
            raise ValueError(
                f"Unsupported encounter_type '{encounter_type}'. "
                f"Supported: {SUPPORTED_ENCOUNTERS}"
            )
        ...
        if encounter_type == "crossing":
            # Place target at ~45° to starboard (ahead-and-right) so it is
            ...
            # Target heads perpendicular: own_heading − 90°
            # i.e. crossing from starboard → port across own ship's bow
    ...
    def get_state(self):
        ...
        if self.encounter_mode:
            ...
            state["encounter_type"] = self.encounter_type
        return state
